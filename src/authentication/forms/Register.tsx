...
export const Register = () => {
  ...
  return (
    <Formik initialValues={{ email: '', password: '', username: '' }} validate={validate} onSubmit={submit}>
      {({ isSubmitting }) => (
        <Form>
          ...
          <Field className={styles.input} id='username' name='username' type='text'/>
          <ErrorMessage component='p' className={styles.error} name='username'/>
          ...
          <button className={styles.button} type='submit' disabled={isSubmitting}>{isSubmitting ? <BarLoader color='#ffffff'/> : 'Register'}</button>
        </Form>
      )}
    </Formik>
  )
